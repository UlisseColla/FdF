...
/*   Updated: 2024/03/12 19:07:49 by ucolla           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	img_init(t_data *img, t_data *menu_img, t_vars_mlx *data)
{
	img->img = mlx_new_image(data->mlx, WIDTH, HEIGHT);
	img->addr = mlx_get_data_addr(img->img, &img->bits_per_pixel, &img->line_length, &img->endian);
	menu_img->img = mlx_new_image(data->mlx, MENU_W, MENU_H);
	menu_img->addr = mlx_get_data_addr(menu_img->img, &menu_img->bits_per_pixel, &menu_img->line_length, &menu_img->endian);
}
...
int	mlx_data_init(t_vars_mlx *data, char *file)
{
	...
	data->menu_win = mlx_new_window(data->mlx, MENU_W, MENU_H, "fdf");
	img_init(&(data->img), &(data->menu_img), data);
	data->has_color = 0;
...
int main(int ac, char **av)
{
	...
	// show_matrix(calculate_y(av[ac - 1]), mlx_data.map);
}
